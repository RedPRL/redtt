...
structure of the NbE algorithm, using closures and explicit shifts
$\uparrow,\downarrow$ in the semantic domain. In this style of NbE,
the evaluation function is called from the quotation function, but not
vice versa.
...
calculate definitional equivalence; worse, later on we will introduce
constructs (such as systems) which resist a sensible notion of normal
form. As such, the quotation algorithm defined below will serve as a
template for a binary generalization that checks definitional
equivalence.
