...
module Q = NewQuote
module D = NewDomain
module C = NewContextual
module U = NewUnify
module R = NewRefiner

module type Import =
sig
  val top_load_file : FileRes.filepath -> unit NewContextual.m
  val top_load_stdin : red : FileRes.filepath -> unit NewContextual.m
  val import : selector : FileRes.selector -> ResEnv.t NewContextual.m
end

module type S =
sig
  val eval_cmd : ML.mlcmd -> ML.semcmd NewContextual.m
end
