...
module Notation = Monad.Notation (NewContextual)
open Notation
...
struct
  ...
  let unleash_pi =
    function
    | D.Pi q -> D.Val.unleash q.dom, q.cod
    | _ -> raise Unleash

  let unleash_sg =
    function
    | D.Sg q -> D.Val.unleash q.dom, q.cod
    | _ -> raise Unleash
  ...
  let quote_ty cx ty =
    Q.equate_tycon (Cx.qenv cx) (Cx.rel cx) ty ty
...
let rec match_spine x0 tw0 sp0 x1 tw1 sp1 =
  let rec go sp0 sp1 =
    ...
    | Snoc (sp0, Tm.FunApp t0), Snoc (sp1, Tm.FunApp t1) ->
      ...
      base_cx >>= fun cx ->
      let dom0, cod0 = Util.unleash_pi ty0 in
      let dom1, cod1 = Util.unleash_pi ty1 in
      let tdom0 = Util.quote_ty cx dom0 in
      let tdom1 = Util.quote_ty cx dom1 in
      active @@ Problem.eqn ~ty0:tdom0 ~ty1:tdom1 ~tm0:t0 ~tm1:t1 >>
      let cod0t0 = Util.inst_clo cx cod0 @@ Ty.eval cx t0 in
      let cod0t1 = Util.inst_clo cx cod1 @@ Ty.eval cx t1 in
      ret (cod0t0, cod0t1)

    | Snoc (sp0, Tm.ExtApp ts0), Snoc (sp1, Tm.ExtApp ts1) ->
      go sp0 sp1 >>= fun (ty0, ty1) ->
      base_cx >>= fun cx ->
      let rs0 = List.map (Ty.eval_dim cx) ts0 in
      let rs1 = List.map (Ty.eval_dim cx) ts1 in
      (* TODO: unify the dimension spines ts0, ts1 *)
      let ty'0, sys0 = Util.unleash_ext_with cx ty0 rs0 in
      let ty'1, sys1 = Util.unleash_ext_with cx ty1 rs1 in
      ret (ty'0, ty'1)

    | Snoc (sp0, Tm.Fst), Snoc (sp1, Tm.Fst) ->
      go sp0 sp1 >>= fun (ty0, ty1) ->
      let dom0, _ = Util.unleash_sg ty0 in
      let dom1, _ = Util.unleash_sg ty1 in
      ret (dom0, dom1)

    | Snoc (sp0, Tm.Snd), Snoc (sp1, Tm.Snd) ->
      go sp0 sp1 >>= fun (ty0, ty1) ->
      base_cx >>= fun cx ->
      let _, cod0 = Util.unleash_sg ty0 in
      let _, cod1 = Util.unleash_sg ty1 in
      let cod0 = Util.inst_clo cx cod0 @@ Ty.eval cx @@ Tm.up (Tm.Var {name = x0; twin = tw0; ushift = 0}, sp0 <>> [Tm.Fst]) in
      let cod1 = Util.inst_clo cx cod1 @@ Ty.eval cx @@ Tm.up (Tm.Var {name = x1; twin = tw1; ushift = 0}, sp1 <>> [Tm.Fst]) in
      ret (cod0, cod1)
...
let normalize_eqn q =
  base_cx >>= fun cx ->
  let vty0 = Ty.eval cx q.ty0 in
  let vty1 = Ty.eval cx q.ty1 in
  let el0 = Ty.eval cx q.tm0 in
  let el1 = Ty.eval cx q.tm1 in
  let tm0 = Util.quote cx vty0 el0 in
  let tm1 = Util.quote cx vty1 el1 in
  let ty0 = Util.quote_ty cx vty0 in
  let ty1 = Util.quote_ty cx vty1 in
  ret {ty0; ty1; tm0; tm1}
...
let unify q =
  ...
  | Tm.Sg (dom0, Tm.B (_, cod0)), Tm.Sg (dom1, Tm.B (_, cod1)) ->
    ...
    let car0 = cmd0 @< Tm.Fst in
    let car1 = cmd1 @< Tm.Fst in
    let cdr0 = Tm.up @@ cmd0 @< Tm.Snd in
    let cdr1 = Tm.up @@ cmd1 @< Tm.Snd in
